	...
	public function sections( $manager ) {
		...
		if ( ( ( ! empty( $shop_isle_wporg_flag ) && ( 'true' === $shop_isle_wporg_flag ) ) ) && ( 'posts' === get_option( 'show_on_front' ) ) ) {
			...
			$manager->add_section( new Shopisle_Customizer_Upsell_Text( $manager, 'shopisle-upsell-frontpage-instructions', array(
				'upsell_text' =>
					sprintf( __( 'To customize the Frontpage sections please create a page. After that, go to %1$s and select "A static page". Finally, for "Front page" choose the page you previously created.', 'shop-isle' ),
					sprintf( '<a class="shop_isle_go_to_section" href="%1$s">%2$s</a>', admin_url( 'customize.php?autofocus[control]=show_on_front' ), esc_html__( 'Static Front Page', 'shop-isle' ) ) ) . '<br><br>' . sprintf( __( 'Need further informations? Check this %1$s', 'shop-isle' ),
